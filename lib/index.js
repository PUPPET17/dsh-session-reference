...
*
* @module dsh-session-reference/resolver
*/
	...
	*/
	async prepare(agent, content, references, signal) {
		...
		const source = {
			...
			form: "recall",
			version: 1,
			label: rendered.map((source) => source.data.label).join(", "),
...
const inject = ["agents", "sessionQuery"];
/** Validated deployment limits for the single host plugin row. */
const Config = SessionReferenceResolver.Config;
...
function apply(ctx, config = {}) {
	...
	ctx.on("agent/pre-step", async ({ agent, signal }, next) => {
		...
		for (const message of decision.messages) {
			...
			if (prepared.additionalContext !== void 0) additional.push(prepared.additionalContext);
			transformed = true;
		}
		if (!transformed) return decision;
		return {
			kind: "enter",
			messages: [...additional, ...out]
		};
	});
}
//#endregion
export { Config, DEFAULT_CANDIDATE_LIMIT, DEFAULT_MAX_REFERENCE_BYTES, MAX_REFERENCES, SESSION_REFERENCE_SCHEME, SessionReferenceError, apply, decodeSessionReferenceUri, encodeSessionReferenceUri, formatSessionReferenceMention, inject, name, parseSessionReferenceText };
